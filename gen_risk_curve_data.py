...
def af_time_data(runs, repeats, l_z, nu_x, nu_yz, angle_vars=False, time=100000):
    data = np.zeros(shape=(runs, repeats, 3), dtype='uint32')
    params = config.settings["structure"]
    params["size"][0], params["seed"] = l_z, None
    params["x_coupling"], params["yz_coupling"] = nu_x, nu_yz
...
def gen_risk(runs, repeats, l_z, nu_x_range, nu_yz_range, angle_vars=False, time=100000, time_data=False):
    ...
    if angle_vars:
        for av in angle_vars[2]:
            result = risk_type(runs, repeats, l_z, 1.0, 1.0, [angle_vars[0], angle_vars[1], av], time)
        ...
        for x in nu_x_range:
            for yz in nu_yz_range:
                result = risk_type(runs, repeats, l_z, x, yz, False, time)
...
if __name__ == '__main__':
    for i in [1, 5, 25]:
        start = time.time()
        gen_risk(runs=20, repeats=1, l_z=i, nu_x_range=[1.0], nu_yz_range=[0.2], angle_vars=False, time=5000)
