...
def gen_risk(runs, repeats, l_z, nu_x_range, nu_yz_range, angle_vars=False,
    ...
    if angle_vars:
        for av in angle_vars[2]:
            result = risk_curve_data(runs, repeats, l_z, 1.0, 1.0, [angle_vars[0], angle_vars[1], av], time)
        ...
        for x in nu_x_range:
            for yz in nu_yz_range:
                result = risk_curve_data(runs, repeats, l_z, x, yz, False, time)
...
if __name__ == '__main__':
    for i in [1, 5, 25]:
        start = time.time()
        gen_risk(runs=100, repeats=1, l_z=i, nu_x_range=[1.0], nu_yz_range=[0.1], angle_vars=False, time=5000)
